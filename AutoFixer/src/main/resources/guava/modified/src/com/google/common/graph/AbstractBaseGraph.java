    ...
    private abstract static class IncidentEdgeSet<N> extends AbstractSet<EndpointPair<N>> {
        ...
        private static final class Directed<N> extends IncidentEdgeSet<N> {
            ...
            public UnmodifiableIterator<EndpointPair<N>> iterator() {
                ...
                }), Iterators.transform(// filter out 'node' from successors (already covered by predecessors, above)
                Sets.difference(graph.successors(node), ImmutableSet.of(node)).iterator(), new Function<N, EndpointPair<N>>() {
        ...
        private static final class Undirected<N> extends IncidentEdgeSet<N> {
            ...
            @Override
            public UnmodifiableIterator<EndpointPair<N>> iterator() {
                return Iterators.unmodifiableIterator(Iterators.transform(graph.adjacentNodes(node).iterator(), new Function<N, EndpointPair<N>>() {
