...
class CompactHashMap<K, V> extends AbstractMap<K, V> implements Serializable {
   ...
   */
     ...
     */
    @VisibleForTesting
    transient int @MonotonicNonNull [] entries;
    ...
    public V put(@Nullable K key, @Nullable V value) {
        ...
        // current size, and pointer to the entry to be appended
    ...
    @Nullable
    private Object removeHelper(@Nullable Object key) {
        if (needsAllocArrays()) {
            return NOT_FOUND;
        }
        int mask = hashTableMask();
        int index = CompactHashing.remove(key, /* value= */
        null, mask, table, entries, keys, /* values= */
        null);
        if (index == -1) {
            return NOT_FOUND;
        }
    ...
    class EntrySetView extends Maps.EntrySet<K, V> {
        ...
        public Spliterator<Entry<K, V>> spliterator() {
            return CollectSpliterators.indexed(size, Spliterator.DISTINCT | Spliterator.ORDERED, MapEntry::new);
        }
        ...
        public boolean remove(@Nullable Object o) {
            if (o instanceof Entry) {
                ...
                int mask = hashTableMask();
                int index = CompactHashing.remove(entry.getKey(), entry.getValue(), mask, table, entries, keys, values);
                if (index == -1) {
                    return false;
                }
                moveLastEntry(index, mask);
