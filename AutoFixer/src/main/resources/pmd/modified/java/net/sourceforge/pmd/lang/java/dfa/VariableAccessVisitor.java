...
public class VariableAccessVisitor extends JavaParserVisitorAdapter {
        ...
        for (Map<VariableNameDeclaration, List<NameOccurrence>> declarations : variableDeclarations) {
            for (Map.Entry<VariableNameDeclaration, List<NameOccurrence>> entry : declarations.entrySet()) {
                ...
                if (vnd.getAccessNodeParent() instanceof ASTFormalParameter) {
                    ...
                    if (vnd.getAccessNodeParent().getFirstDescendantOfType(ASTVariableInitializer.class) != null) {
                ...
                for (NameOccurrence occurrence : entry.getValue()) {
                    ...
                    Node potentialAssignment = occurrence.getLocation().getFirstParentOfAnyType(ASTStatementExpression.class, ASTExpression.class);
                    while (potentialAssignment != null && (potentialAssignment.jjtGetNumChildren() < 2 || !(potentialAssignment.jjtGetChild(1) instanceof ASTAssignmentOperator))) {
                        potentialAssignment = potentialAssignment.getFirstParentOfAnyType(ASTStatementExpression.class, ASTExpression.class);
                    }
    ...
    private void addAccess(JavaNameOccurrence occurrence, DataFlowNode inode) {
        if (occurrence.isOnLeftHandSide()) {
            ...
            if (occurrence.isOnRightHandSide() || !occurrence.isOnLeftHandSide() && !occurrence.isOnRightHandSide()) {
