...
public class GuardLogStatementRule extends AbstractJavaRule implements Rule {
     ...
     * severe  -> isLoggable
     */
    private static final PropertyDescriptor<List<String>> LOG_LEVELS = stringListProperty("logLevels").desc("LogLevels to guard").defaultValues("trace", "debug", "info", "warn", "error", "log", "finest", "finer", "fine", "info", "warning", "severe").delim(',').build();

    private static final PropertyDescriptor<List<String>> GUARD_METHODS = stringListProperty("guardsMethods").desc("Method use to guard the log statement").defaultValues("isTraceEnabled", "isDebugEnabled", "isInfoEnabled", "isWarnEnabled", "isErrorEnabled", "isLoggable").delim(',').build();
    ...
    public Object visit(ASTStatementExpression node, Object data) {
        ...
        if (primary.jjtGetNumChildren() >= 2 && primary.jjtGetChild(0) instanceof ASTPrimaryPrefix) {
            ...
            if (guardStmtByLogLevel.containsKey(methodCall) && logLevel != null && primary.jjtGetChild(1) instanceof ASTPrimarySuffix && primary.jjtGetChild(1).hasDescendantOfType(ASTAdditiveExpression.class)) {
    ...
    private String getLogLevelName(Node node, String methodCallName) {
        ...
        if (suffix != null) {
            ...
            if (argumentList != null && argumentList.jjtGetNumChildren() > 0) {
                ...
                ASTName name = GuardLogStatementRule.<ASTName>getFirstChild(argumentList.jjtGetChild(0), ASTPrimaryExpression.class, ASTPrimaryPrefix.class, ASTName.class);
