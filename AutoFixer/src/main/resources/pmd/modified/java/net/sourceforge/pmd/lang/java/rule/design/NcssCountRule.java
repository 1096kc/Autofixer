...
public final class NcssCountRule extends AbstractJavaMetricsRule {
    ...
    private static final PropertyDescriptor<Integer> METHOD_REPORT_LEVEL_DESCRIPTOR = PropertyFactory.intProperty("methodReportLevel").desc("NCSS reporting threshold for methods").require(positive()).defaultValue(60).build();

    private static final PropertyDescriptor<Integer> CLASS_REPORT_LEVEL_DESCRIPTOR = PropertyFactory.intProperty("classReportLevel").desc("NCSS reporting threshold for classes").require(positive()).defaultValue(1500).build();
    ...
    private static final PropertyDescriptor<List<NcssOption>> NCSS_OPTIONS_DESCRIPTOR = PropertyFactory.enumListProperty("ncssOptions", OPTION_MAP).desc("Choose options for the computation of Ncss").emptyDefaultValue().build();
    ...
    public Object visit(ASTAnyTypeDeclaration node, Object data) {
        ...
        if (JavaClassMetricKey.NCSS.supports(node)) {
            ...
            if (classSize >= classReportLevel) {
                String[] messageParams = { node.getTypeKind().name().toLowerCase(Locale.ROOT), node.getImage(), classSize + " (Highest = " + classHighest + ")" };
    ...
    public Object visit(ASTMethodOrConstructorDeclaration node, Object data) {
        ...
        if (methodSize >= methodReportLevel) {
            addViolation(data, node, new String[] { node instanceof ASTMethodDeclaration ? "method" : "constructor", node.getQualifiedName().getOperation(), "" + methodSize });
        }
        return data;
    }
}
