    ...
    public boolean suppresses(Rule rule) {
        ...
        if (isSuppressWarnings()) {
            for (ASTLiteral element : findDescendantsOfType(ASTLiteral.class)) {
                if (element.hasImageEqualTo("\"PMD\"") || element.hasImageEqualTo("\"PMD." + rule.getName() + "\"") || // Check for standard annotations values
                element.hasImageEqualTo("\"all\"") || element.hasImageEqualTo("\"serial\"") && SERIAL_RULES.contains(rule.getName()) || element.hasImageEqualTo("\"unused\"") && UNUSED_RULES.contains(rule.getName())) {
                    return true;
                }
            }
        }
