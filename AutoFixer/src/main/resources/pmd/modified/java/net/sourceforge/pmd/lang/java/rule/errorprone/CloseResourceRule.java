...
public class CloseResourceRule extends AbstractJavaRule {
    ...
    private static final PropertyDescriptor<List<String>> CLOSE_TARGETS_DESCRIPTOR = stringListProperty("closeTargets").desc("Methods which may close this resource").emptyDefaultValue().delim(',').build();

    private static final PropertyDescriptor<List<String>> TYPES_DESCRIPTOR = stringListProperty("types").desc("Affected types").defaultValues("java.lang.AutoCloseable", "java.sql.Connection", "java.sql.Statement", "java.sql.ResultSet").delim(',').build();

    private static final PropertyDescriptor<Boolean> USE_CLOSE_AS_DEFAULT_TARGET = booleanProperty("closeAsDefaultTarget").desc("Consider 'close' as a target by default").defaultValue(true).build();

    private static final PropertyDescriptor<List<String>> ALLOWED_RESOURCE_TYPES = stringListProperty("allowedResourceTypes").desc("Exact class names that do not need to be closed").defaultValues("java.io.ByteArrayOutputStream", "java.io.ByteArrayInputStream", "java.io.StringWriter", "java.io.CharArrayWriter", "java.util.stream.Stream").build();
    ...
    private boolean isMethodParameter(ASTVariableDeclarator var, ASTMethodOrConstructorDeclaration methodOrCstor) {
        ...
        if (name != null) {
            ...
            if (methodOrCstor.getKind() == MethodLikeKind.METHOD) {
                ...
                if (methodOrCstor.getKind() == MethodLikeKind.CONSTRUCTOR) {
    ...
    private boolean isResourceTypeOrSubtype(TypeNode refType) {
        if (refType.getType() != null) {
            ...
            if (refType.jjtGetNumChildren() > 0 && refType.jjtGetChild(0) instanceof ASTReferenceType) {
        ...
        for (ASTTryStatement t : tryblocks) {
            ...
            if (!hasNullInitializer(var) && parentBlock.jjtGetParent() == tryBlock.jjtGetParent()) {
                ...
                for (int i = parentBlockIndex + 1; i < tryBlockIndex; i++) {
                    ...
                    ASTLocalVariableDeclaration varDecl = block.getFirstDescendantOfType(ASTLocalVariableDeclaration.class);
                    ASTStatementExpression statementExpression = block.getFirstDescendantOfType(ASTStatementExpression.class);
                    if (varDecl == null && (statementExpression == null || statementExpression.getFirstChildOfType(ASTAssignmentOperator.class) == null)) {
