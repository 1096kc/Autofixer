...
public class LawOfDemeterRule extends AbstractJavaRule {
    ...
    private static class MethodCall {
        ...
        private boolean isNotBuilder() {
            return baseType != StringBuffer.class && baseType != StringBuilder.class && !"StringBuilder".equals(baseTypeName) && !"StringBuffer".equals(baseTypeName) && !methodName.endsWith("Builder");
        }
        ...
        private void checkViolation() {
            ...
            if (baseNameInWhitelist) {
                ...
                if (SCOPE_LOCAL.equals(baseScope)) {
                    ...
                    if (SCOPE_METHOD_CHAINING.equals(baseScope)) {
                        violation = true;
                        violationReason = REASON_METHOD_CHAIN_CALLS;
                    } else {
                        if (SCOPE_STATIC_CHAIN.equals(baseScope)) {
                            violation = true;
        ...
        private void determineType() {
            ...
            if (var instanceof TypedNameDeclaration) {
                ...
                if (METHOD_CALL_CHAIN.equals(baseName)) {
                    baseScope = SCOPE_METHOD_CHAINING;
                } else {
                    if (baseName.contains(".") && !baseName.startsWith("System.")) {
        ...
        private Assignment determineLastAssignment() {
            ...
            for (ASTAssignmentOperator stmt : assignmentStmts) {
                ...
                if (stmt.getBeginLine() <= expression.getBeginLine() && stmt.hasImageEqualTo(SIMPLE_ASSIGNMENT_OPERATOR)) {
                    // now we need to make sure it has the right image name
                    ASTPrimaryPrefix primaryPrefix = stmt.jjtGetParent().getFirstDescendantOfType(ASTPrimaryPrefix.class);
        ...
        private boolean isIterator() {
            ...
            if (baseType != null && baseType == Iterator.class || baseTypeName != null && baseTypeName.endsWith("Iterator")) {
                iterator = true;
            }
            return iterator;
        }
    ...
    private static class Assignment implements Comparable<Assignment> {
        ...
        @Override
        public String toString() {
            return "assignment: line=" + line + " allocation:" + allocation + " iterator:" + iterator + " forLoop: " + forLoop;
        }
