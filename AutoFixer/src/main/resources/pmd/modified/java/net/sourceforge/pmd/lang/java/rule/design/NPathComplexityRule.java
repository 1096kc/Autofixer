...
public class NPathComplexityRule extends AbstractJavaMetricsRule {
    ...
    @Deprecated
    private static final PropertyDescriptor<Double> MINIMUM_DESCRIPTOR = PropertyFactory.doubleProperty("minimum").desc("Deprecated! Minimum reporting threshold").require(positive()).defaultValue(200d).build();

    private static final PropertyDescriptor<Integer> REPORT_LEVEL_DESCRIPTOR = PropertyFactory.intProperty("reportLevel").desc("N-Path Complexity reporting threshold").require(positive()).defaultValue(200).build();
    ...
    public final Object visit(ASTMethodOrConstructorDeclaration node, Object data) {
        ...
        if (npath >= reportLevel) {
            addViolation(data, node, new String[] { node instanceof ASTMethodDeclaration ? "method" : "constructor", node.getQualifiedName().getOperation(), "" + npath });
        }
        return data;
    }
}
