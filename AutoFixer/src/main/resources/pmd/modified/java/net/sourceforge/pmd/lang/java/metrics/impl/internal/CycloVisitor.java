...
public class CycloVisitor extends JavaParserVisitorAdapter {
    ...
    public Object visit(ASTSwitchStatement node, Object data) {
        ...
        for (ASTSwitchLabel label : node) {
            ...
            if (considerBooleanPaths) {
                ...
                if (node.jjtGetNumChildren() > 1 + label.jjtGetChildIndex() && node.jjtGetChild(label.jjtGetChildIndex() + 1) instanceof ASTBlockStatement) {
    ...
    public Object visit(ASTAssertStatement node, Object data) {
        if (considerAssert) {
            // equivalent to if (condition) { throw .. }
