...
public final class MethodTypeResolution {
    ...
    static {
        ...
        // defining a local variable to suppress warnings only for
        @SuppressWarnings("PMD.AvoidUsingShortType")
        Class<?> // the following statement
        ...
        // this is here for convenience, not really in order
        primitiveList.add(char.class);
    ...
    public static MethodType selectAmongMaximallySpecific(MethodType first, MethodType second) {
        if (first.isAbstract()) {
            if (second.isAbstract()) {
                ...
                // second one isn't abstract
                return second;
            }
        } else {
            if (second.isAbstract()) {
                // first isn't abstract, second one is
                return first;
            } else {
                // TODO: once shadowing and overriding methods is done, add exception back
                return first;
    ...
    public static boolean isMethodApplicable(Method method, String methodName, int argArity, Class<?> accessingClass, List<JavaTypeDefinition> typeArguments) {
        return // name matches
        method.getName().equals(methodName) && // is visible
        isMemberVisibleFromClass(method.getDeclaringClass(), method.getModifiers(), accessingClass) && // if method is vararg with arity n, then the invocation's arity >= n - 1
        (!method.isVarArgs() || argArity >= getArity(method) - 1) && // if the method isn't vararg, then arity matches
        (method.isVarArgs() || argArity == getArity(method)) && // isn't generic or arity of type arguments matches that of parameters
        (!isGeneric(method) || typeArguments.isEmpty() || method.getTypeParameters().length == typeArguments.size());
    }
    ...
    public static boolean isMemberVisibleFromClass(Class<?> classWithMember, int modifiers, Class<?> accessingClass) {
        ...
        if (accessingClass.getPackage() != null) {
            // if null, then it's in the default package
    ...
    public static boolean isMethodConvertible(JavaTypeDefinition parameter, JavaTypeDefinition argument) {
        ...
        if (// arg is primitive
        indexInPrimitive != -1 && isSubtypeable(parameter, JavaTypeDefinition.forClass(BOXED_PRIMITIVE_SUBTYPE_ORDER.get(indexInPrimitive)))) {
            return true;
        }
        // covers unboxing
        int indexInBoxed = BOXED_PRIMITIVE_SUBTYPE_ORDER.indexOf(argument.getType());
        return // arg is boxed primitive
        indexInBoxed != -1 && isSubtypeable(parameter, JavaTypeDefinition.forClass(PRIMITIVE_SUBTYPE_ORDER.get(indexInBoxed)));
        ...
        if (indexOfParameter != -1) {
            if (argument.getType() == char.class) {
                if (indexOfParameter <= 3) {
                    // <= 3 because short and byte are not compatible with char
                    return true;
                }
            } else {
