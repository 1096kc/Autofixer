...
public class ConsecutiveLiteralAppendsRule extends AbstractJavaRule {
    ...
    private static final PropertyDescriptor<Integer> THRESHOLD_DESCRIPTOR = PropertyFactory.intProperty("threshold").desc("Max consecutive appends").require(inRange(1, 10)).defaultValue(1).build();
            ...
            if (InefficientStringBufferingRule.isInStringBufferOperation(n, 3, "append")) {
                ...
                for (int jx = 0; jx < numChildren; jx++) {
                    ...
                    if (isAdditive(sn)) {
                        ...
                        if (!isAppendingStringLiteral(sn)) {
                            checkForViolation(rootNode, data, concurrentCount);
                            concurrentCount = 0;
                        } else {
                            concurrentCount++;
                ...
                if (n.getImage().endsWith(".toString") || n.getImage().endsWith(".length")) {
        ...
        for (int i = 0; i < primary.jjtGetNumChildren(); i++) {
            ...
            if (child instanceof ASTPrimarySuffix) {
                ...
                if (suffix.jjtGetNumChildren() == 0 && suffix.hasImageEqualTo("append")) {
                    ...
                    if (suffix.jjtGetNumChildren() > 0 && previousWasAppend) {
    ...
    private Node getFirstParentBlock(Node node) {
        ...
        if (parentNode instanceof ASTIfStatement) {
            ...
            if (parentNode instanceof ASTSwitchStatement) {
