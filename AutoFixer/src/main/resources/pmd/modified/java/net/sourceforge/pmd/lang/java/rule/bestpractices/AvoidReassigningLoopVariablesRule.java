...
public class AvoidReassigningLoopVariablesRule extends AbstractOptimizationRule {
    ...
    private static final PropertyDescriptor<ForeachReassignOption> FOREACH_REASSIGN = enumProperty("foreachReassign", FOREACH_REASSIGN_VALUES).defaultValue(ForeachReassignOption.DENY).desc("how/if foreach control variables may be reassigned").build();
    ...
    private static final PropertyDescriptor<ForReassignOption> FOR_REASSIGN = enumProperty("forReassign", FOR_REASSIGN_VALUES).defaultValue(ForReassignOption.DENY).desc("how/if for control variables may be reassigned").build();
        ...
        if (node.jjtGetParent() instanceof ASTForInit) {
            ...
            if (node.jjtGetParent() instanceof ASTForStatement) {
                ...
                if (foreachReassign == ForeachReassignOption.FIRST_ONLY) {
                    ...
                    if (foreachReassign == ForeachReassignOption.DENY) {
                        checkAssignExceptIncrement(data, loopVariables, loopBody);
                        checkIncrementAndDecrement(data, loopVariables, loopBody);
                    }
                }
    ...
    private enum IgnoreFlags {
        ...
        IGNORE_FIRST, IGNORE_CONDITIONAL
    }
}
