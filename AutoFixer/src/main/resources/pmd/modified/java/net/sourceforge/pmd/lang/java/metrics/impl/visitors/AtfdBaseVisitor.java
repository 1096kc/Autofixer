...
public class AtfdBaseVisitor extends JavaParserVisitorAdapter {
    ...
    private boolean isForeignAttributeOrMethod(ASTPrimaryExpression node) {
        ...
        if (nameImage != null && (!nameImage.contains(".") || nameImage.startsWith("this."))) {
            ...
            if (nameImage == null && node.getFirstDescendantOfType(ASTPrimaryPrefix.class).usesThisModifier()) {
                result = false;
            } else {
                if (nameImage == null && node.hasDescendantOfAnyType(ASTLiteral.class, ASTAllocationExpression.class)) {
                    result = false;
                } else {
                    result = true;
                }
