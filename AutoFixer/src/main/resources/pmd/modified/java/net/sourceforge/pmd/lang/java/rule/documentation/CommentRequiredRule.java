...
public class CommentRequiredRule extends AbstractCommentRule {
    ...
    private static final PropertyDescriptor<CommentRequirement> ACCESSOR_CMT_DESCRIPTOR = requirementPropertyBuilder("accessorCommentRequirement", "Comments on getters and setters\"").defaultValue(CommentRequirement.Ignored).build();

    private static final PropertyDescriptor<CommentRequirement> OVERRIDE_CMT_DESCRIPTOR = requirementPropertyBuilder("methodWithOverrideCommentRequirement", "Comments on @Override methods").defaultValue(CommentRequirement.Ignored).build();

    private static final PropertyDescriptor<CommentRequirement> HEADER_CMT_REQUIREMENT_DESCRIPTOR = requirementPropertyBuilder("headerCommentRequirement", "Header comments").build();
    ...
    private static final PropertyDescriptor<CommentRequirement> SERIAL_VERSION_UID_CMT_REQUIREMENT_DESCRIPTOR = requirementPropertyBuilder("serialVersionUIDCommentRequired", "Serial version UID comments").defaultValue(CommentRequirement.Ignored).build();

    private static final PropertyDescriptor<CommentRequirement> SERIAL_PERSISTENT_FIELDS_CMT_REQUIREMENT_DESCRIPTOR = requirementPropertyBuilder("serialPersistentFieldsCommentRequired", "Serial persistent fields comments").defaultValue(CommentRequirement.Ignored).build();
    ...
    public Object visit(ASTMethodDeclaration decl, Object data) {
        if (isAnnotatedOverride(decl)) {
            ...
            if (decl.getSignature().role == JavaOperationSignature.Role.GETTER_OR_SETTER) {
    ...
    private boolean isAnnotatedOverride(ASTMethodDeclaration decl) {
        ...
        for (ASTMarkerAnnotation ann : annotations) {
            // TODO consider making a method to get the annotations of a method
    ...
    public Object visit(ASTFieldDeclaration decl, Object data) {
        if (isSerialVersionUID(decl)) {
            ...
            if (isSerialPersistentFields(decl)) {
    ...
    private boolean isSerialPersistentFields(final ASTFieldDeclaration field) {
        return "serialPersistentFields".equals(field.getVariableName()) && field.isPrivate() && field.isStatic() && field.isFinal() && field.isArray() && // .getType() returns null
        "ObjectStreamField".equals(field.jjtGetFirstToken().getImage());
    }
