...
public class ClassTypeResolver extends JavaParserVisitorAdapter {
    ...
    private JavaTypeDefinition getFieldType(JavaTypeDefinition typeToSearch, String fieldImage, Class<?> accessingClass) {
        while (typeToSearch != null && typeToSearch.getType() != Object.class) {
            try {
                ...
                if (LOG.isLoggable(Level.WARNING)) {
                    String message = "Error during type resolution of field '" + fieldImage + "' in " + typeToSearch.getType() + " due to: " + e;
            ...
            if (type == null) {
                ...
                if (expression != null && expression.getTypeDefinition() != null) {
                    ...
                    if (typeDefinitionIterable.isArrayType()) {
                        ...
                        if (typeDefinitionIterable.isGeneric() && typeDefinitionIterable.getGenericType(0) != null) {
        ...
        for (int childIndex = 0; childIndex < primaryNode.jjtGetNumChildren(); ++childIndex) {
            ...
            if (currentChild.getType() == null) {
                ...
                // Last token, because if 'super' is a Suffix, it'll have tokens '.' and 'super'
                    ...
                    if ("super".equals(currentChild.jjtGetLastToken().toString())) {
                        if (previousChild != null) {
                        ...
                        if (currentChild.getFirstChildOfType(ASTArguments.class) != null) {
                            currentChild.setTypeDefinition(previousChild.getTypeDefinition());
                        } else {
                            if (previousChild != null && previousChild.getType() != null) {
    ...
    private TypeNode getEnclosingTypeDeclaration(Node node) {
        ...
        while (node != null) {
            ...
            // anonymous class declaration
                ...
                if (// is anonymous class declaration
    ...
    private JavaTypeDefinition getSuperClassTypeDefinition(Node node, Class<?> clazz) {
        ...
        for (; node != null; previousNode = node, node = node.jjtGetParent()) {
            ...
            node instanceof ASTClassOrInterfaceDeclaration && // is the class we are looking for or caller requested first class
            ...
            // anonymous class declaration
                ...
                if (// callers requested any class scope
                clazz == null && // is anonymous class decl
                node instanceof ASTAllocationExpression && // arrays can't be anonymous
                node.getFirstChildOfType(ASTArrayDimsAndInits.class) == null && !(previousNode instanceof ASTArguments)) {
                    // we might come out of the constructor
    ...
    public Object visit(ASTTypeParameter node, Object data) {
        if (!node.hasTypeBound()) {
            // type parameter doesn't have declared upper bounds
    ...
    public Object visit(ASTSwitchExpression node, Object data) {
        ...
        for (ASTSwitchLabeledRule rule : rules) {
            // second child is either Expression, Block, ThrowStatement
            Node body = rule.jjtGetChild(1);
    ...
    private ASTTypeParameter getTypeParameterDeclaration(Node startNode, String image) {
        for (Node parent = startNode.jjtGetParent(); parent != null; parent = parent.jjtGetParent()) {
            ...
            if (parent instanceof ASTTypeParameters) {
                // if type parameter defined in the same < >
                typeParameters = (ASTTypeParameters) parent;
            } else {
                if (parent instanceof ASTConstructorDeclaration || parent instanceof ASTMethodDeclaration || parent instanceof ASTClassOrInterfaceDeclaration) {
    ...
    public Class<?> loadClass(String fullyQualifiedClassName) {
        try {
            ...
            if (LOG.isLoggable(Level.FINE)) {
                LOG.log(Level.FINE, "Tried to load class " + fullyQualifiedClassName + " from on demand import, " + "with an incomplete classpath.", e2);
            }
            return null;
        }
    }
