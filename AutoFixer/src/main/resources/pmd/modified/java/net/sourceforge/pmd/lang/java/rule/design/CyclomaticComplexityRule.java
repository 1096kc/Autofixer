...
public class CyclomaticComplexityRule extends AbstractJavaMetricsRule {
    ...
    @Deprecated
    private static final PropertyDescriptor<Integer> REPORT_LEVEL_DESCRIPTOR = PropertyFactory.intProperty("reportLevel").desc("Deprecated! Cyclomatic Complexity reporting threshold").require(positive()).defaultValue(10).build();

    private static final PropertyDescriptor<Integer> CLASS_LEVEL_DESCRIPTOR = PropertyFactory.intProperty("classReportLevel").desc("Total class complexity reporting threshold").require(positive()).defaultValue(80).build();

    private static final PropertyDescriptor<Integer> METHOD_LEVEL_DESCRIPTOR = PropertyFactory.intProperty("methodReportLevel").desc("Cyclomatic complexity reporting threshold").require(positive()).defaultValue(10).build();
    ...
    private static final PropertyDescriptor<List<CycloOption>> CYCLO_OPTIONS_DESCRIPTOR = PropertyFactory.enumListProperty("cycloOptions", OPTION_MAP).desc("Choose options for the computation of Cyclo").emptyDefaultValue().build();
    ...
    public Object visit(ASTAnyTypeDeclaration node, Object data) {
        ...
        if (JavaClassMetricKey.WMC.supports(node)) {
            ...
            if (classWmc >= classReportLevel) {
                ...
                String[] messageParams = { node.getTypeKind().getPrintableName(), node.getImage(), " total", classWmc + " (highest " + classHighest + ")" };
    ...
    public final Object visit(MethodLikeNode node, Object data) {
        ...
        if (cyclo >= methodReportLevel) {
            addViolation(data, node, new String[] { node.getKind().getPrintableName(), node.getQualifiedName().getOperation(), "", "" + cyclo });
        }
        return data;
    }
}
