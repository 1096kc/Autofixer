...
public class UnnecessaryFullyQualifiedNameRule extends AbstractJavaRule {
    ...
    private void checkImports(TypeNode node, Object data) {
                ...
                if (importDeclaration.isStatic()) {
                    ...
                    if (!importDeclaration.isImportOnDemand()) {
                        ...
                        if (nameParts[nameParts.length - 1].equals(importParts[importParts.length - 1])) {
                            ...
                            if (couldBeMethodCall(node) && nameParts.length > 1 && nameParts[nameParts.length - 2].equals(importParts[importParts.length - 1])) {
        ...
        if (matches.isEmpty()) {
            ...
            if (!isReferencingInnerNonStaticClass(name, firstMatch) && !isAvoidingConflict(node, name, firstMatch)) {
    ...
    private boolean isSamePackage(TypeNode node, String name) {
        if (node.getType() != null) {
            ...
            if (packageOfType != null) {
                ...
                return node.getType().getPackage().getName().equals(currentPackage) && name.equals(currentPackage);
            }
        }
        int i = name.lastIndexOf('.');
        if (i > 0) {
        ...
        if (node.jjtGetParent() instanceof ASTPrimaryPrefix || node.jjtGetParent() instanceof ASTNameList || node instanceof ASTClassOrInterfaceType) {
    ...
    private boolean isJavaLangImplicit(TypeNode node) {
        ...
        if (isJavaLang && node.getType() != null && node.getType().getPackage() != null) {
            ...
            String packageName = // package might be null, if type is an array type...
            node.getType().getPackage().getName();
    ...
    private boolean isAvoidingConflict(final TypeNode node, final String name, final ASTImportDeclaration firstMatch) {
            ...
            for (final ASTImportDeclaration importDeclaration : imports) {
                if (!Objects.equals(importDeclaration, firstMatch) && importDeclaration.isStatic()) {
                    ...
                    if (importDeclaration.isImportOnDemand()) {
                        ...
                        if (importDeclaration.getImportedName().endsWith(methodCalled)) {
                            return true;
                        }
                    }
                }
        ...
        if (firstMatch.isImportOnDemand() && !firstMatch.isStatic()) {
            for (ASTImportDeclaration importDeclaration : imports) {
                if (// NOPMD
                importDeclaration != firstMatch && !importDeclaration.isStatic() && !importDeclaration.isImportOnDemand()) {
