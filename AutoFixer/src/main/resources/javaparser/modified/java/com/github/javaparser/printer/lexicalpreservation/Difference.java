...
public class Difference {
    ...
    private void applyRemovedDiffElement(RemovedGroup removedGroup, Removed removed, TextElement originalElement, boolean originalElementIsChild, boolean originalElementIsToken) {
        if (removed.isChild() && originalElementIsChild) {
            ...
            if (removed.isToken() && originalElementIsToken && (removed.getTokenType() == ((TokenTextElement) originalElement).getTokenKind() || // handle EOLs separately as their token kind might not be equal. This is because the 'removed'
            ((TokenTextElement) originalElement).getToken().getCategory().isEndOfLine() && removed.isNewLine())) {
                ...
                if (originalElementIsToken && originalElement.isWhiteSpaceOrComment()) {
                    ...
                    if (removed.isPrimitiveType()) {
                        ...
                        if (removed.isWhiteSpace() || removed.getElement() instanceof CsmIndent || removed.getElement() instanceof CsmUnindent) {
                            diffIndex++;
                        } else {
            ...
            if (indentation.isPresent() && !isReplaced(lastElementIndex)) {
                for (int i = 0; i < indentation.get(); i++) {
                    if (originalElements.get(originalIndex).isSpaceOrTab()) {
                        ...
                        if (originalIndex >= 1 && originalElements.get(originalIndex - 1).isSpaceOrTab()) {
                ...
                if (kept.isChild() && originalElementIsToken) {
                    ...
                    if (kept.isToken() && originalElementIsToken) {
                        ...
                        if (kept.getTokenType() == originalTextToken.getTokenKind()) {
                            ...
                            if (kept.isNewLine() && originalTextToken.isSpaceOrTab()) {
                                originalIndex++;
                                diffIndex++;
                            ...
                            // see issue 2351
                                ...
                                if (!kept.isNewLine() && originalTextToken.isSeparator()) {
                                    originalIndex++;
                                } else {
    ...
    private void applyAddedDiffElement(Added added) {
        ...
        if (originalIndex > 0 && originalElements.get(originalIndex - 1).isNewline()) {
            ...
            if (isAfterLBrace(nodeText, originalIndex) && !isAReplacement(diffIndex)) {
            ...
            for (int i = startIndex; i < nodeText.getElements().size(); i++) {
                if (!correspondingIndices.contains(i)) {
                    ...
                    if (isCorresponding) {
                        ...
                        if (isAlmostCorrespondingElement(textElement, csmElement, node)) {
